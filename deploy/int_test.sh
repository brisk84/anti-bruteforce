...
./deploy/ab-client ab 192.168.1.1 >/tmp/ab.out

./deploy/ab-client l User2 Pass1 192.168.1.1 >/tmp/ab.out
fileEquals /tmp/ab.out "${expected_false}"

./deploy/ab-client db 192.168.1.1 >/tmp/ab.out
...
for ((i=0; i < 100; i++))
do
    ./deploy/ab-client l User_$i Pass2 192.168.1.2 >/tmp/ab.out
    fileEquals /tmp/ab.out "${expected_true}"
done
./deploy/ab-client l User$i Pass2 192.168.1.2 >/tmp/ab.out
fileEquals /tmp/ab.out "${expected_false}"

./deploy/ab-client aw 192.168.1.1 >/tmp/ab.out
